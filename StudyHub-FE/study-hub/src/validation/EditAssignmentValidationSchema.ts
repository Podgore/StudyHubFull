...
import type { CreateAssignment } from "../components/AssignmentBuilder/NewAssignmentModal";
...
export const editAssignmentValidationSchema = yup.object<CreateAssignment>().shape({
    title: yup.string().required("Title is required"),
    kind: yup.number().oneOf([0, 1]).required(),
    maxMark: yup.number().typeError("Max mark must be a number").min(0).required("Max mark is required"),
    openingDate: yup
        .string()
    ...
    instructions: yup.string().max(20000).optional(),
    lectureId: yup.string().optional(),
});
