import * as React from 'react';
import Button from '@mui/material/Button';
import IconButton from '@mui/material/IconButton';
import useNotification from '../../hooks/useNotification';
import { Box, FormControl, InputLabel, MenuItem, Select, TextField, Typography } from '@mui/material';
import EditOutlinedIcon from '@mui/icons-material/EditOutlined';
import DescriptionOutlinedIcon from '@mui/icons-material/DescriptionOutlined';
import EventOutlinedIcon from '@mui/icons-material/EventOutlined';
import AccessTimeOutlinedIcon from '@mui/icons-material/AccessTimeOutlined';
import { DateTimePicker, LocalizationProvider } from '@mui/x-date-pickers';
import { AdapterDayjs } from '@mui/x-date-pickers/AdapterDayjs';
import dayjs from 'dayjs';
import { parseDurationHhMmSsToMs, pickerDayjsToOffsetIsoString } from '../../utils/assignmentDateTimeForm';
import Assignment from '../../api/Assignment';
import { useForm, type Resolver } from 'react-hook-form';
import { yupResolver } from '@hookform/resolvers/yup';
import { editAssignmentValidationSchema } from '../../validation/EditAssignmentValidationSchema';
import { UpdateAssignmentRequest } from '../../api/models/request/Assignment/UpdateAssignmentRequest';
import AssignmentResponse, { AssignmentKind } from '../../api/models/response/AssignmentResponse';
import { CreateAssignment } from './NewAssignmentModal';
import Lecture from '../../api/Lecture';
import type { LectureResponse } from '../../api/models/response/LectureResponse';
import {
    AssignmentFormModalLayout,
    AssignmentModalFieldRow,
} from './AssignmentFormModalLayout';

export function assignmentDurationToFormString(raw: unknown): string {
    if (raw == null) return '00:00:00';
    if (typeof raw === 'string') {
        const timePart = raw.includes('.') ? raw.split('.').pop()! : raw;
        const m = timePart.match(/^(\d+):(\d{2}):(\d{2})/);
        if (m) {
            const h = m[1].padStart(2, '0');
            return `${h}:${m[2]}:${m[3]}`;
        }
        return raw;
    }
    if (raw instanceof Date) {
        return [
            String(raw.getUTCHours()).padStart(2, '0'),
            String(raw.getUTCMinutes()).padStart(2, '0'),
            String(raw.getUTCSeconds()).padStart(2, '0'),
        ].join(':');
    }
    return '00:00:00';
}

interface EditAssignmentModalProps {
    assignment: AssignmentResponse;
    onUpdated: () => void;
    trigger?: 'icon' | 'button';
}

const popperStopPropagation = {
    popper: {
        onClick: (e: React.MouseEvent) => e.stopPropagation(),
        onMouseDown: (e: React.MouseEvent) => e.stopPropagation(),
    },
};

const fieldInputSx = {
    '& .MuiOutlinedInput-root': {
        borderRadius: '8px',
    },
};

const EditAssignmentModal = ({ assignment, onUpdated, trigger = 'icon' }: EditAssignmentModalProps) => {
    const [open, setOpen] = React.useState(false);
    const [lectures, setLectures] = React.useState<LectureResponse[]>([]);
    const { notifySuccess } = useNotification();

    const opening = new Date(assignment.openingDate);
    const canEdit = opening > new Date();

    const {
        register,
        handleSubmit,
        formState: { errors },
        setValue,
        watch,
        reset,
        trigger: triggerFieldValidation,
    } = useForm<CreateAssignment>({
        resolver: yupResolver(editAssignmentValidationSchema) as Resolver<CreateAssignment>,
        mode: 'onTouched',
        reValidateMode: 'onChange',
    });

    const kind = assignment.kind ?? AssignmentKind.TimedTest;

    const kindWatch = watch('kind');
    const openingDateWatch = watch('openingDate');
    const durationWatch = watch('duration');

    const closingMinDateTime = React.useMemo(() => {
        if (!openingDateWatch) return dayjs();
        const open = dayjs(openingDateWatch);
        if (!open.isValid()) return dayjs();
        if (kindWatch === AssignmentKind.Homework) {
            return open.add(1, 'second');
        }
        const durMs = parseDurationHhMmSsToMs(durationWatch || '00:00:00');
        if (durMs == null || durMs <= 0) return open.add(1, 'second');
        return open.add(durMs, 'millisecond');
    }, [openingDateWatch, durationWatch, kindWatch]);

    const durationRegister = register('duration');

    const handleOpen = async () => {
        const list = await Lecture.getLecturesBySubject(String(assignment.subjectId));
        setLectures(list ?? []);
        reset({
            title: assignment.title,
            maxMark: assignment.maxMark,
            openingDate: pickerDayjsToOffsetIsoString(dayjs(assignment.openingDate)),
            closingDate: pickerDayjsToOffsetIsoString(dayjs(assignment.closingDate)),
            duration: assignmentDurationToFormString(assignment.duration),
            kind,
            instructions: assignment.instructions ?? '',
            lectureId: assignment.lectureId ? String(assignment.lectureId) : '',
        });
        setOpen(true);
    };

    const handleClose = () => setOpen(false);

    const handleUpdateAssignment = async (data: CreateAssignment) => {
        try {
            const payload: UpdateAssignmentRequest = {
                title: data.title,
                maxMark: Number(data.maxMark),
                openingDate: data.openingDate,
                closingDate: data.closingDate,
                duration: data.duration,
                instructions: kind === AssignmentKind.Homework ? data.instructions || null : null,
                lectureId:
                    kind === AssignmentKind.Homework && data.lectureId ? data.lectureId : null,
            };

            const result = await Assignment.updateAssignment(String(assignment.id), payload);
            if (result) {
                notifySuccess('Assignment updated successfully.');
                handleClose();
                onUpdated();
            }
        } catch {
            /* Server message shown via global API handler */
        }
    };

    return (
        <>
            <AssignmentFormModalLayout
                open={open}
                onClose={handleClose}
                title="Edit Assignment"
                mode="edit"
                primaryLabel="Save Changes"
                onPrimaryClick={() => handleSubmit(handleUpdateAssignment)()}
                primaryDisabled={!canEdit}
            >
                <LocalizationProvider dateAdapter={AdapterDayjs}>
                    <AssignmentModalFieldRow icon={<DescriptionOutlinedIcon />} label="Name">
                        <TextField
                            id="edit-assignment-name"
                            variant="outlined"
                            fullWidth
                            placeholder="Assignment name"
                            sx={fieldInputSx}
                            {...register('title')}
                            error={!!errors.title}
                            helperText={errors.title?.message}
                        />
                    </AssignmentModalFieldRow>

                    <AssignmentModalFieldRow
                        icon={
                            <Typography
                                component="span"
                                sx={{ fontSize: '1.1rem', fontWeight: 700, color: 'text.secondary', lineHeight: 1 }}
                            >
                                #
                            </Typography>
                        }
                        label="Mark"
                    >
                        <TextField
                            id="edit-assignment-mark"
                            variant="outlined"
                            fullWidth
                            type="number"
                            sx={fieldInputSx}
                            {...register('maxMark')}
                            error={!!errors.maxMark}
                            helperText={errors.maxMark?.message}
                        />
                    </AssignmentModalFieldRow>

                    <Box
                        sx={{
                            display: 'flex',
                            flexDirection: { xs: 'column', sm: 'row' },
                            gap: 2,
                            mb: 2.5,
                        }}
                    >
                        <Box sx={{ flex: 1, minWidth: 0 }}>
                            <AssignmentModalFieldRow icon={<EventOutlinedIcon />} label="Assignment start">
                                <DateTimePicker
                                    minDateTime={dayjs()}
                                    value={watch('openingDate') ? dayjs(watch('openingDate')) : null}
                                    onChange={(newValue) => {
                                        setValue('openingDate', newValue ? pickerDayjsToOffsetIsoString(newValue) : '', {
                                            shouldValidate: true,
                                            shouldDirty: true,
                                        });
                                        void triggerFieldValidation('closingDate');
                                    }}
                                    slotProps={{
                                        ...popperStopPropagation,
                                        textField: {
                                            fullWidth: true,
                                            variant: 'outlined',
                                            sx: fieldInputSx,
                                            error: !!errors.openingDate,
                                            helperText: errors.openingDate?.message,
                                        },
                                    }}
                                />
                            </AssignmentModalFieldRow>
                        </Box>
                        <Box sx={{ flex: 1, minWidth: 0 }}>
                            <AssignmentModalFieldRow icon={<EventOutlinedIcon />} label="Assignment end">
                                <DateTimePicker
                                    minDateTime={closingMinDateTime}
                                    value={watch('closingDate') ? dayjs(watch('closingDate')) : null}
                                    onChange={(newValue) =>
                                        setValue('closingDate', newValue ? pickerDayjsToOffsetIsoString(newValue) : '', {
                                            shouldValidate: true,
                                            shouldDirty: true,
                                        })
                                    }
                                    slotProps={{
                                        ...popperStopPropagation,
                                        textField: {
                                            fullWidth: true,
                                            variant: 'outlined',
                                            sx: fieldInputSx,
                                            error: !!errors.closingDate,
                                            helperText: errors.closingDate?.message,
                                        },
                                    }}
                                />
                            </AssignmentModalFieldRow>
                        </Box>
                    </Box>

                    <AssignmentModalFieldRow icon={<AccessTimeOutlinedIcon />} label="Duration (hh:mm:ss)">
                        <TextField
                            id="edit-assignment-duration"
                            variant="outlined"
                            fullWidth
                            placeholder="02:30:00"
                            sx={fieldInputSx}
                            {...durationRegister}
                            onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                                durationRegister.onChange(e);
                                if (kind === AssignmentKind.TimedTest) void triggerFieldValidation('closingDate');
                            }}
                            error={!!errors.duration}
                            disabled={kind === AssignmentKind.Homework}
                            helperText={
                                errors.duration?.message ??
                                (kind === AssignmentKind.Homework
                                    ? 'Not used for homework'
                                    : 'Format: hours:minutes:seconds (e.g., 02:30:00)')
                            }
                        />
                    </AssignmentModalFieldRow>

                    {kind === AssignmentKind.Homework && (
                        <>
                            <AssignmentModalFieldRow icon={<DescriptionOutlinedIcon />} label="Linked lecture">
                                <FormControl fullWidth sx={fieldInputSx}>
                                    <InputLabel id="edit-assignment-lecture-label">Optional</InputLabel>
                                    <Select
                                        labelId="edit-assignment-lecture-label"
                                        label="Optional"
                                        value={watch('lectureId') || ''}
                                        onChange={(e) =>
                                            setValue('lectureId', e.target.value as string, {
                                                shouldValidate: true,
                                            })
                                        }
                                    >
                                        <MenuItem value="">
                                            <em>None</em>
                                        </MenuItem>
                                        {lectures.map((lec) => (
                                            <MenuItem key={lec.id} value={String(lec.id)}>
                                                {lec.title}
                                            </MenuItem>
                                        ))}
                                    </Select>
                                </FormControl>
                            </AssignmentModalFieldRow>
                            <AssignmentModalFieldRow icon={<DescriptionOutlinedIcon />} label="Instructions">
                                <TextField
                                    variant="outlined"
                                    fullWidth
                                    multiline
                                    minRows={3}
                                    sx={fieldInputSx}
                                    {...register('instructions')}
                                    error={!!errors.instructions}
                                    helperText={errors.instructions?.message}
                                />
                            </AssignmentModalFieldRow>
                        </>
                    )}
                </LocalizationProvider>
            </AssignmentFormModalLayout>

            {trigger === 'icon' ? (
                <IconButton
                    size="small"
                    disabled={!canEdit}
                    aria-label="Edit assignment"
                    title={canEdit ? 'Edit assignment details' : 'Cannot edit after the assignment has opened'}
                    onClick={(e) => {
                        e.stopPropagation();
                        if (canEdit) handleOpen();
                    }}
                    sx={{ color: 'text.secondary' }}
                >
                    <EditOutlinedIcon fontSize="small" />
                </IconButton>
            ) : (
                <Button
                    variant="outlined"
                    size="small"
                    disabled={!canEdit}
                    title={canEdit ? 'Edit assignment details' : 'Cannot edit after the assignment has opened'}
                    onClick={(e) => {
                        e.stopPropagation();
                        if (canEdit) handleOpen();
                    }}
                >
                    Edit
                </Button>
            )}
        </>
    );
};

export default EditAssignmentModal;
